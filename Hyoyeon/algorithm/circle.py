...
def solution(n, weak, dist):
    ...
    for i in range(weak_l):
        start = [weak[j] for j in range(i, i+weak_l)] #[10, 13, 17, 18] 시작점 하나씩 바꾸기
    
        for dist_p in dist_comb:
            result = 1 # 활용 친구 수
            dist_distance = 0
            check_len = start[0] + dist_p[0]   #start point=10에서 dist경우수 [1,2,3,4]로 ch_len=11,12,13,14 
            #print(check_len)
        
            for k in range(weak_l):
                if start[k] > check_len: #확인 가능한 최대 거리를 넘겼을 경우
                    result += 1
